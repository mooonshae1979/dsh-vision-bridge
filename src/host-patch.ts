...
function applyPatch(source: string): string {
  if (!source.includes(ANCHOR_IMPORT)) throw new Error('missing top import anchor')
  if (!source.includes(ANCHOR_PROMPT)) throw new Error('missing prompt admission anchor')
  if (!source.includes(ANCHOR_SELECT)) throw new Error('missing selectModel admission anchor')
 ...
 */
export async function autoApplyHostPatch(ctx: Context, config: HostPatchConfig): Promise<void> {
  ...
  ctx.logger.warn(
    'dsh-vision-bridge: dsh-host-apiproxy lib/index.js not found; host patch not applied. ' +
