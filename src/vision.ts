...
import type { ModelSelectionConfig, SelectedVisionRoute } from './models.js'
...
/** One user message's images together with the text that accompanied them. */
interface ImageEntry {
  images: ImageBlock[]
  /** The user's own words from the same message (their actual question/instruction). */
  text: string
}
 ...
 */
function collectImageEntries(messages: readonly UserMessage[]): ImageEntry[] {
...
function buildTranscriptionPrompt(entries: readonly ImageEntry[], settings: VisionSettings): ContentBlock[] {
  const blocks: ContentBlock[] = []
  entries.forEach((entry, index) => {
    for (const image of entry.images) blocks.push({ ...image })
    if (settings.includeUserText && entry.text.length > 0) {
      blocks.push({
        type: 'text',
        text:
          `【第${index + 1}组图片的用户附言/需求】\n${entry.text}\n` +
...
function bareAttachmentId(attachmentId: string): string {
...
function attachmentObjectPath(attachmentId: string): string | undefined {
  try {
    ...
    if (id.length < 2) return undefined
...
function describeImage(ref: ImageAttachmentRef): string {
  const parts: string[] = []
  if (ref.name !== undefined && ref.name.length > 0) parts.push(ref.name)
  parts.push(`${ref.width}x${ref.height}`)
  if (ref.bytes >= 1024) parts.push(`${Math.round(ref.bytes / 102.4) / 10}KB`)
...
function failurePlaceholder(
  reason: string,
  routes: readonly SelectedVisionRoute[],
...
function replaceImages(blocks: readonly ContentBlock[], text: string): ContentBlock[] {
  let first = true
...
async function modelSeesImages(
  ctx: Context,
  agent: Agent,
 ...
 */
async function transcribeViaSubagent(
  ctx: Context,
  parent: Agent,
  entries: ImageEntry[],
  settings: VisionSettings,
  ...
  const prompt: ContentBlock[] = buildTranscriptionPrompt(entries, settings)
  ...
  for (let i = 0; i < Math.min(attempts, routes.length); i++) {
    ...
    const onAbort = (): void => controller.abort(signal.reason ?? 'parent aborted')
    signal.addEventListener('abort', onAbort, { once: true })
    const timer = setTimeout(
      () => controller.abort('transcribe timeout'),
      settings.transcribeTimeoutMs,
    )
    let run
    try {
  ...
  if (settings.fallbackToDirect && !signal.aborted) {
    ...
    if (!direct.startsWith('【图片转写失败')) return direct
...
export function installVision(ctx: Context, settings: () => VisionSettings): void {
  const nativeVisionCache = new Map<string, boolean>()
  ctx.on('agent/pre-step', async (payload, next) => {
    ...
    if (await modelSeesImages(ctx, payload.agent, nativeVisionCache)) return decision
    ...
    const rewritten: UserMessage[] = decision.messages.map((message) =>
      message.content.some(isImage)
        ? { ...message, content: replaceImages(message.content, text) }
        : message,
    )
    return { kind: 'enter', messages: rewritten } satisfies PreStepDecision
  })
}
